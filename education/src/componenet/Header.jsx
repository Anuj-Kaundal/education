...
function Header() {
    ...
    return (
        <div className='bg-[#DDE9FF]'>
            ...
            {/* Mobile Menu */}
            <div
                className={`lg:hidden bg-[#F0F0F0] overflow-hidden transition-all duration-500 ${menu ? 'max-h-[700px] opacity-100 pt-10 pb-10' : 'max-h-0 opacity-0'
                    }`}
            >
                ...
                <ul className='flex flex-col items-center gap-6 mt-6 text-base font-medium'>
                    <li onClick={() => { navigate('/');setMenu(false);}}>Home</li>
                    <li onClick={() => { navigate('/course');setMenu(false);}}>Courses</li>
                    <li onClick={() => { navigate('/internship');setMenu(false);}}>Internships</li>
                    <li onClick={() => { navigate('/success-stories');setMenu(false);}}>Success Stories</li>
                    <li onClick={() => { navigate('/about');setMenu(false);}}>About</li>
                    <li onClick={() => { navigate('/contact');setMenu(false);}}>Contact</li>
                </ul>

                {/* Buttons */}
                <div className='flex flex-col items-center gap-4 mt-6'>
                    <button className='text-[#3746F2] font-medium' onClick={() => { navigate('/login');setMenu(false);}}>Login</button>
                    <button className='bg-[#3746F2] text-white px-4 py-2 rounded-md' onClick={() => { navigate('/register');setMenu(false);}}>
                        Register
                    </button>
                </div>
            </div>
